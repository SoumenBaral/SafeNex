    ...
    <AdminLayout>
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="font-display font-bold text-white text-2xl">Rescue Teams</h1>
                    <p class="text-ink-400 text-sm mt-1">Manage teams and assign them to verified incidents</p>
                </div>
            ...
            <!-- Empty state -->
            <div v-if="!teams.data.length" class="bg-ink-800 rounded-xl border border-ink-600 py-16 flex flex-col items-center justify-center">
                <p class="text-3xl mb-3">🚑</p>
                <p class="text-white font-semibold font-display">No rescue teams yet</p>
                <p class="text-ink-400 text-sm mt-1">Add your first team to start dispatching responders.</p>
            ...
            <div v-else class="grid sm:grid-cols-2 xl:grid-cols-3 gap-5">
                <div
                    ...
                    class="bg-ink-800 rounded-xl border border-ink-600 p-5 hover:border-ink-500 transition-all"
                >
                    ...
                    <!-- Leader -->
                    <div class="flex items-center gap-2.5 mb-4 pb-4 border-b border-ink-700">
                        ...
                        <div class="min-w-0">
                            <p class="text-white text-sm font-medium truncate">{{ team.leader_name }}</p>
                            <p class="text-ink-400 text-xs font-data truncate">{{ team.leader_email }}</p>
                        </div>
                    </div>

                    <!-- Members -->
                    <div class="space-y-1.5 mb-4">
                        <p class="text-ink-400 text-xs font-semibold uppercase tracking-wider">
                            {{ team.members?.length ?? 0 }} member{{ (team.members?.length ?? 0) !== 1 ? 's' : '' }}
                        </p>
                        <div v-if="team.members?.length" class="space-y-1">
                            <p v-for="m in team.members.slice(0, 3)" :key="m.id" class="text-ink-300 text-xs truncate">
                                {{ m.name }}<span v-if="m.role_in_team" class="text-ink-500"> · {{ m.role_in_team }}</span>
                            </p>
                            <p v-if="team.members.length > 3" class="text-ink-500 text-xs">+{{ team.members.length - 3 }} more</p>
                        </div>
                    </div>
