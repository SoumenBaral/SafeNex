...
<template>
    ...
    <AdminLayout>
        <div class="space-y-6">
            <div>
                <h1 class="font-display font-bold text-gray-900 text-2xl">Emergency Alerts</h1>
                <p class="text-gray-500 text-sm mt-1">Broadcast emergency notifications to users in targeted districts</p>
            </div>
            ...
            <div class="grid lg:grid-cols-[1fr_360px] gap-6 items-start">
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="px-5 py-4 border-b border-gray-200">
                        <h2 class="font-display font-semibold text-gray-900 text-sm">Sent alerts</h2>
                    </div>
                    <div v-if="!alerts.data.length" class="py-12 flex flex-col items-center justify-center">
                        <p class="text-3xl mb-3">📢</p>
                        <p class="text-gray-500 text-sm">No alerts sent yet</p>
                    </div>
                    <div v-else class="divide-y divide-gray-200">
                        <div v-for="alert in alerts.data" :key="alert.id" class="px-5 py-4">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1 min-w-0">
                                    <p class="text-gray-900 font-medium text-sm">{{ alert.title }}</p>
                                    <p class="text-gray-500 text-xs mt-1 leading-relaxed">{{ alert.message }}</p>
                                    <div class="flex items-center gap-3 mt-2 font-data text-xs text-gray-400">
                ...
                <!-- Compose form (sticky) -->
                <div class="bg-white rounded-xl border border-[#D62839]/30 shadow-sm overflow-hidden lg:sticky lg:top-6">
                    <div class="px-5 py-4 border-b border-gray-200 flex items-center gap-2">
                        <span class="text-base">🚨</span>
                        <h2 class="font-display font-semibold text-gray-900 text-sm">Compose alert</h2>
                    </div>
                    <form @submit.prevent="showConfirm = true" class="p-5 space-y-4">
                        <div>
                            <label class="block text-gray-600 text-xs font-medium mb-1.5">Alert title <span class="text-[#D62839]">*</span></label>
                            <input v-model="form.title" type="text"
                                class="w-full bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-sm px-3 py-2 placeholder-gray-400 focus:border-bay-500 focus:ring-0"
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-gray-600 text-xs font-medium mb-1.5">Target district</label>
                                <select v-model="form.district_id" @change="form.upazila_id = ''"
                                    class="w-full bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-xs px-3 py-2 focus:border-bay-500 focus:ring-0">
                                    <option value="">All districts</option>
                                    <option v-for="d in districts" :key="d.id" :value="d.id">{{ d.name }}</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-gray-600 text-xs font-medium mb-1.5">Target upazila</label>
                                <select v-model="form.upazila_id" :disabled="!upazilas.length"
                                    class="w-full bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-xs px-3 py-2 focus:border-bay-500 focus:ring-0 disabled:opacity-50">
                        ...
                        <div>
                            <label class="block text-gray-600 text-xs font-medium mb-1.5">Message <span class="text-[#D62839]">*</span></label>
                        ...
                        <!-- Reach preview -->
                        <div v-if="form.district_id" class="bg-gray-50 rounded-lg px-3 py-2.5 border border-gray-200">
                            <p class="text-gray-500 text-xs">
                                <span class="text-gray-900 font-medium">Target:</span>
                                {{ selectedDistrict?.name }}
                                <template v-if="form.upazila_id"> / {{ upazilas.find(u => u.id == form.upazila_id)?.name }}</template>
                            </p>
                        </div>
                        <div v-else class="bg-gray-50 rounded-lg px-3 py-2.5 border border-gray-200">
                            <p class="text-gray-500 text-xs"><span class="text-gray-900 font-medium">Target:</span> All users nationwide</p>
                        </div>
